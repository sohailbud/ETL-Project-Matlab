function Agency = importLocalAgenciesUpdated(p)

path=strcat(p,'Data\ReadIn\Local Agencies Old');
...
Agency.Heinrich = fixHeinrichDates(Agency.Heinrich);
    ...
    function Agency = insertMissingColumnsFixVariableNames(Agency)
        ...
        for fields = fieldnames(Agency)'
            for field = fieldnames(Agency.(fields{1}))'
                ...
                try d.Properties.VariableNames{'Sponsorship'}='Sponsorships'; catch; end
                try d.Properties.VariableNames{'SPONSORSHIP'}='Sponsorships'; catch; end
                try d.Properties.VariableNames{'WEEKOF'}='Week'; catch; end
                try d.Properties.VariableNames{'WeekOf'}='Week'; catch; end
                try d.Properties.VariableNames{'WkOf'}='Week'; catch; end
                try d.Properties.VariableNames{'WeekEnding'}='Week'; catch; end
                try d.WeeklyTotal=[]; catch; end
                try d.Total=[]; catch; end
...
%% Heinrich DATE FIX
%  Change 'Saturday' to previous 'Monday'
    function D = fixHeinrichDates(D)
        D.Days=cellstr(datestr(D.Week,'dddd'));
        D.Week=datenum(D.Week);
        D.Week(strcmp(D.Days,'Saturday'))=D.Week(strcmp(D.Days,'Saturday'))-5;
        D.Days=[];
    end
            ...
            for vn=VN
                if ~strcmp(vn{1},'Week') && ~strcmp(vn{1},'DMA_MKT_NAME')
                    if round(nansum(Agency.(field{1}).(vn{1})))~=...
                        ...
                        field{1}
                        vn{1}
                        round(nansum(Agency.(field{1}).(vn{1})))
                        round(nansum(AgencyRaw.(field{1}).(vn{1})))
